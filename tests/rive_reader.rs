...
#[test] fn parse_rive_file() -> Result<()> {
    ...
    visit_assets_dir(Path::new(assets_dir), &mut |path| {
        if path.extension().is_none_or(|s| s != "riv") { return Ok(()) } else { files += 1; }
        println!("\nParsing Rive file: {}", path.display());
        let rive_file = RiveFile::read(&mut BufReader::new(fs::File::open(path)?))?;
    ...
    assert!(0 < files, "no Rive fixtures were found");  Ok(())
}
